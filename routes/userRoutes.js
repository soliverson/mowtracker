...
// Get upcoming week range (today to 7 days ahead)
...
const upcomingJobs = jobs
  .filter(job => {
    ...
    if (job.time) {
      const [hour = '00', minute = '00'] = job.time.split(':');
      jobDate.setHours(parseInt(hour), parseInt(minute));
    } else {
...
// Calendar view with customer dropdown
  ...
  userModel.getJobs(req.session.user.id, (err, jobs) => {
    ...
    userModel.getCustomers(req.session.user.id, (err2, customers) => {
      ...
      const upcomingJobs = jobs.filter(job =>
        new Date(`${job.date}T${job.time || '00:00'}`) >= now
      ).sort((a, b) =>
...
// View jobs for a specific customer
...
// Add new appointment page (optional pre-filled customer)
...
router.post('/add-appointment', (req, res) => {
  ...
  userModel.addJob(req.session.user.id, customer_name, date, amount_paid || 0, phone, notes, address, time, (err) => {
...
router.get('/reports', (req, res) => {
  ...
  userModel.getJobs(userId, (err, jobs) => {
    ...
    const monthJobs = jobs.filter(job => {
      const jobDate = new Date(job.date);
      return jobDate >= new Date(startOfMonth) && jobDate <= new Date(endOfMonth);
    });

    const monthTotal = monthJobs.reduce((sum, job) => sum + parseFloat(job.amount_paid || 0), 0);
    const ytdJobs = jobs.filter(job => new Date(job.date).getFullYear() === parseInt(selectedYear));
